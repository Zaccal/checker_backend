...
describe('DELETE method', () => {
    ...
    test('Delete subtask with non-existent ID should return 404', async () => {
        ...
        const mockError = new Prisma.PrismaClientKnownRequestError('Record to delete does not exist.', {
            code: 'P2025',
            clientVersion: '6.0.0',
        });
        // Create a mock error that will pass instanceof check
        mockSubTask.delete.mockRejectedValueOnce(mockError);
        const response = await appInstance.request('/api/v1/subtasks/non-existent-id', {
            method: 'DELETE',
            headers: globalThis.authHeader,
            credentials: 'include',
        });
