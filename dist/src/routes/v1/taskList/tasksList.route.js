...
// GET
tasksList.get('/search', zValidator('query', SearchQuerySchema, (result, c) => {
    if (!result.success) {
...
// POST
tasksList.post('/', zValidator('json', createListSchema, (result, c) => {
    if (!result.success) {
...
// PATCH
tasksList.patch('/:id', zValidator('json', updateListSchema, (result, c) => {
    if (!result.success) {
        return c.text('Invalid request body', 400);
    }
}), async (c) => {
